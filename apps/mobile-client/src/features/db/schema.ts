...
export const exportsTable = sqliteTable('export', {
...
export const canonicalPeople = sqliteTable('canonical_person', {
  id: integer('id').primaryKey(),
...
export const canonicalConversations = sqliteTable(
  ...
  (t) => [check('ck_canonical_conversation_type', sql`${t.type} in ('dm','group')`)],
)
...
export const canonicalConversationsRelations = relations(canonicalConversations, ({ many }) => ({
  conversations: many(conversations),
}))
