...
export const mixColors = (baseColor: string, mixColor: string, ratio: number) => {
  ...
  const mixedRed = Math.round(baseRed * ratio + mixRed * (1 - ratio))
  const mixedGreen = Math.round(baseGreen * ratio + mixGreen * (1 - ratio))
  const mixedBlue = Math.round(baseBlue * ratio + mixBlue * (1 - ratio))
