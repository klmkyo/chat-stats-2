...
- Merge tools live in `/merge-suggestions` (auto DM grouping) and `/merge-conversations` (manual merge with preview + rename). Launch from the chats header buttons or via long-press context menu. Only direct messages are eligible; group chats stay read-only. Merges run through React Query `useMutation`, and each transaction finishes with `DELETE FROM canonical_conversation WHERE NOT EXISTS (...)` to drop unused rows. Skip `useInvalidateDb()` unless Rust mutates the database.
- Auto merge suggestions are filtered for duplicate sources per merge and respect the ignore list stored under `AUTO_MERGE_IGNORED` (see `features/chats/merge/ignoredSuggestions.ts`). `useIgnoredSuggestionKeys()` returns `{ keys, ignore, unignore, clear }`. Toggling a suggestion off adds it to the ignored set (hidden from the Chats CTA); toggling back on removes it. Settings > Suggested merges is always available for review.
- Use `useAutoMergeSuggestions(chats)` to memoize merge heuristics via React Query; it returns the usual `{ data, isLoading }` shape.
- `useDbQuery` wraps our own multi-table live query helper (in `features/db/hooks/useDb.ts`) that listens for join/with table names because Expo’s `useLiveQuery` currently ignores joins. It now exposes `{ data, error, isLoading, updatedAt }`; use `isLoading` for spinners while Drizzle reruns the query. Pass any extra dependencies as a stable array if you need to force a rerun.
- Debug page includes “Reset chats database”, which mirrors the corruption recovery flow by setting `DB_RESET_PENDING` and reloading via `DevSettings.reload()`.
